...
from .lbs import lbs, vertices2landmarks
from .graphics import face_vertices
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        # add faces and uvs
        _, _, vertex_uvs, faces_verts_idx, _, face_uvs_idx, _, _, _, = load_obj(flame_template_mesh_path)
        vertex_uvs = torch.tensor(vertex_uvs)
        face_uvs_idx = torch.tensor(face_uvs_idx) - 1
        faces_verts_idx = torch.tensor(faces_verts_idx) - 1
        ...
        self.register_buffer("faces", faces_verts_idx, persistent=False)
